...
- $L_2$-regularization : $\Omega(\v w)=\lambda\norm{\v w}_2^2$ with $\norm{\v w}_2^2=\sum_{j=0}^M w_j^2$ and $\lambda$ regularization parameter
  ...
  - ridge regression : $L_2$-regularization with MSE, - explicit solution : $\v w_{ridge}^*=(\v\Phi^\top\v\Phi+2N\lambda\v I_M)^{-1}\v\Phi^\top\v y$
...
- trainging error : $L_S(f_S)=\frac{1}{N}\sum_{n=1}^N l(y_n,f(\v x_n))$, empirical
  - isse as a function of itself : $L_S(f_S)=\frac{1}{N}\sum_{n=1}^N l(y_n,f_S(\v x_n))$
