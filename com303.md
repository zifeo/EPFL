...
- **discrete fourier series** : DFS, $N$-periodic, equivalent to DFT of one period
  - analysis : $X[k]=\frac{1}{N}\sum_{k=0}^{N-1} x[n]e^{-j\frac{2\pi}{N}nk}$ with $n\in\Z$
...
- **z-transform** : power series, convergence always absolute
  ...
  - region of convergence : ROC, circular symmetry
    ...
    - $p_n$ : poles of transfert function, push sheet up
      ![](img/com303-24.png)
      ![](img/com303-25.png)
  - constant coefficient difference equation : $\sum_{k=0}^{N-1}a_ky[n-k]=\sum_{k=0}^{M-1}b_kx[n-k]$ give $Y(z)\sum_{k=0}^{N-1}a_k=X(z)\sum_{k=0}^{M-1}$ with $Y(z)=H(z)X(z)$
...
- **limitations**
  ...
  - magnitude response cannot by constant over an interval : $B(z)-cA(z)$ infinite number of roots, need to be $0$ for all $z$
